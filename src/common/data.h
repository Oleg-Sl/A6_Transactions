...
namespace s21 {
	...
	public:
		...
		std::string ToString() {
			std::stringstream ss;
			ss << std::left << std::setw(15) << name << std::left << std::setw(15) << surname
				<< std::left << std::setw(10) << std::to_string(birthday) << std::left << std::setw(15)
				<< city << std::left << std::setw(8) << std::to_string(coins);
			return ss.str();
		}

		bool IsEmpty() const {
			if (name == "" && surname == "" && birthday == 0 && city == "" && coins == 0) return true;
			return false;
		}
	};
