...
class Tree {
 public:
  ...
  bool Insert(const Key& key, const Value& value) {
    ...
    auto middle_it = std::lower_bound(leaf->keys.begin(), leaf->keys.end(), key);
 ...
 private:
  ...
  void MoveDataToNewNode(Node* src, Node* dst, size_t border) {
    if (src->is_leaf) {
      ...
      // std::for_each(dst->children.begin(), dst->children.end(), [&dst](auto& item) { item->parent = dst; });
      src->children.erase(src->children.begin() + border + 1, src->children.end());
    }
  }
