...
//         auto middle_it = lower_bound(leaf->keys.begin(), leaf->keys.end(),
//         key); size_t insert_index = middle_it - leaf->keys.begin();
...
//             leaf->values.push_back(value);
//             leaf->keys.push_back(key);
//             std::rotate(leaf->keys.begin() + insert_index, leaf->keys.end() -
//             1, leaf->keys.end()); std::rotate(leaf->values.begin() +
//             insert_index, leaf->values.end() - 1, leaf->values.end());
//         }
...
//             // std::cout << "Remove root node if it is empty: " << key <<
//             std::endl; updateRoot(node); return;
//         }
//         // printTree();
//         // если узел заполнен не оптимально
//         if (node->keys.size() < degree) {
//             // std::cout << "remove key and rebalance = " << key <<
//             std::endl; rebalance(node);
//         } else {
...
//         // std::cout << "============== REBALANCE ==============" <<
//         std::endl; node_type* left_neighbor = node->left; node_type*
//         right_neighbor = node->right;
//         // printTree();
...
//             left_neighbor->keys.back());
//             node->values.insert(node->values.begin(),
//             left_neighbor->values.back()); left_neighbor->keys.pop_back();
//             left_neighbor->values.pop_back();
//             recursiveUpdateKeys(node->parent);
//         } else if (right_neighbor != nullptr && right_neighbor->keys.size() >
//         degree) {
...
//             if (node->is_leaf) {
//                 std::move(node->keys.begin(), node->keys.end(),
//                 std::back_inserter(left_neighbor->keys));
//                 std::move(node->values.begin(), node->values.end(),
//                 std::back_inserter(left_neighbor->values));
//             } else {
//                 std::transform(node->childs.begin(), node->childs.end(),
//                 std::back_inserter(left_neighbor->keys), [](auto item) {
//                 return item->keys.front(); });
//                 std::move(node->childs.begin(), node->childs.end(),
//                 std::back_inserter(left_neighbor->childs)); for (auto& item :
//                 left_neighbor->childs) {
...
//             // left_neighbor.end()
//             // printTree();
//             auto it = upper_bound(node->parent->keys.begin(),
//             node->parent->keys.end(), *left_neighbor->keys.begin());
...
//             if (node->is_leaf) {
//                 std::move(right_neighbor->keys.begin(),
//                 right_neighbor->keys.end(), std::back_inserter(node->keys));
//                 std::move(right_neighbor->values.begin(),
//                 right_neighbor->values.end(),
//                 std::back_inserter(node->values));
//             } else {
//                 std::transform(right_neighbor->childs.begin(),
//                 right_neighbor->childs.end(), std::back_inserter(node->keys),
//                 [](auto item) { return item->keys.front(); });
//                 std::move(right_neighbor->childs.begin(),
//                 right_neighbor->childs.end(),
//                 std::back_inserter(node->childs)); for (auto& item :
//                 node->childs) {
//                     item->parent = node;
//                 }
//             }
...
//             // printTree();
//             auto it = upper_bound(right_neighbor->parent->keys.begin(),
//             right_neighbor->parent->keys.end(), *node->keys.begin());
...
//         *new_node->keys.begin()); size_t mid_index = mid_it -
//         parent->keys.begin(); parent->keys.push_back(mid_key);
//         parent->childs.push_back(new_node);
//         std::rotate(parent->keys.begin() + mid_index, parent->keys.end() - 1,
//         parent->keys.end()); std::rotate(parent->childs.begin() + mid_index +
//         1, parent->childs.end() - 1, parent->childs.end());
//         // printNode(parent);
...
//         // std::cout << "moveDataToNewNode src = " << src << ", dst = " <<
//         dst << ", border = " << border << std::endl;
//         std::move(src->keys.begin() + border, src->keys.end(),
//         std::back_inserter(dst->keys)); src->keys.erase(src->keys.begin() +
//         border, src->keys.end()); if (src->is_leaf) {
//             std::move(src->values.begin() + border, src->values.end(),
//             std::back_inserter(dst->values));
//             src->values.erase(src->values.begin() + border,
//             src->values.end());
//         } else {
//             // std::transform(src->childs.begin() + border,
//             src->childs.end(), src->childs.begin() + border, [dst](auto item)
//             { return item->parent = dst; });
//             // auto tmp = dst->childs.end() - 1;
//             std::move(src->childs.begin() + border, src->childs.end(),
//             std::back_inserter(dst->childs)); for (auto& item : dst->childs)
//             {
//                 item->parent = dst;
//             }
//             // std::transform(dst->childs.begin(), dst->childs.end(),
//             dst->childs.begin(), [dst](auto item) { return item->parent =
//             dst; });
//             // std::transform(tmp, dst->childs.end(), dst, [dst](auto item) {
//             return item->parent = dst; });
//             // std::transform(src->childs.begin(), src->childs.end(),
//             src->childs.begin(), [dst](auto item) { return item->parent =
//             dst; }); src->childs.erase(src->childs.begin() + border,
//             src->childs.end());
//         }
//     }
...
//         ")" << std::endl; std::cout << "Parent: " << node->parent <<
//         std::endl; std::cout << "keys = "; for (size_t i = 0; i <
//         node->keys.size(); ++i) {
