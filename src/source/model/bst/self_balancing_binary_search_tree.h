...
namespace s21 {
	...
	SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::SelfBalancingBinarySearchTree() {
		root_ = nullptr;
		leaf_ = new Node();
	}
		...
		if (!root_) {
			...
			if (Exists(key)) {
				...
				while (current_node != leaf_) {
					parent = current_node;
					current_node = (current_node->data.first > key)
						? current_node->link[0]
						: current_node->link[1];
				}
				current_node = tmp;
	...
	template<typename Key, typename Value, typename ValueEqual>
	Value SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::Get(const Key& key) const {
		Pointer current_node = root_;
	...
	template<typename Key, typename Value, typename ValueEqual>
	bool SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::Exists(const Key& key) const {
	...
	template<typename Key, typename Value, typename ValueEqual>
	bool SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::Del(const Key& key) {
	...
	template<typename Key, typename Value, typename ValueEqual>
	bool SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::Update(const Key& key, const Value& value) {
		Pointer node = Search(key);
		if (!node) return false;
	...
	template<typename Key, typename Value, typename ValueEqual>
	std::vector<Key> SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::Keys() const {
		std::vector<Key> keys;
		Iterator i = Begin();
		if (i.IsValid()) {
			while (i != End()) {
	...
	template<typename Key, typename Value, typename ValueEqual>
	bool SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::Rename(const Key& key, const Key& new_key) {
		Pointer node = Search(key);
		if (!node) return false;
	...
	template<typename Key, typename Value, typename ValueEqual>
	std::vector<Key> SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::Find(const Value& value) const {
		std::vector<Key> keys;
		Iterator i = Begin();
		if (i.IsValid()) {
			ValueEqual comparator;
			while (i != End()) {
				if (comparator(i->second, value))
					keys.push_back(i->first);
				++i;
			}
			if (comparator(i->second, value))
	...
	template<typename Key, typename Value, typename ValueEqual>
	std::vector<Value> SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::Showall() const {
	...
	template<typename Key, typename Value, typename ValueEqual>
	BSTNode<Key, Value>* SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::Search(const Key& key) const {
		Pointer node = root_;
	...
	template<typename Key, typename Value, typename ValueEqual>
	bool SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::RemoveNode(Pointer node) {
	...
	template<typename Key, typename Value, typename ValueEqual>
	int SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::CountChildren(Pointer node) const {
		int count = 0;
		if (node) {
	...
	template<typename Key, typename Value, typename ValueEqual>
	void SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::RemoveNodeWithoutChildren(Pointer node) {
	...
	template<typename Key, typename Value, typename ValueEqual>
	void SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::RemoveNodeWithOneChild(Pointer node) {
	...
	template<typename Key, typename Value, typename ValueEqual>
	void SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::RemoveNodeWithTwoChild(Pointer node) {
	...
	template<typename Key, typename Value, typename ValueEqual>
	BSTNode<Key, Value>* SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::GetParent(Pointer node) const {
		return node->parent;
	}

	template<typename Key, typename Value, typename ValueEqual>
	BSTNode<Key, Value>* SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::GetChild(Pointer node) const {
		Pointer child = node->link[0] != leaf_ ? node->link[0] : node->link[1];
		return child;
	}

	template<typename Key, typename Value, typename ValueEqual>
	void SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::Swap(Pointer a, Pointer b) {
	...
	template<typename Key, typename Value, typename ValueEqual>
	void SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::RebalanceTree(Pointer node) {
	...
	template<typename Key, typename Value, typename ValueEqual>
	bool SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::NodeIsLeftChild(Pointer node) {
		return GetParent(node)->link[0] == node;
	}

	template<typename Key, typename Value, typename ValueEqual>
	void SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::LeftRotation(Pointer node) {
	...
	template<typename Key, typename Value, typename ValueEqual>
	void SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::RightRotation(Pointer node) {
	...
	template<typename Key, typename Value, typename ValueEqual>
	BSTNode<Key, Value>* SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::MinValueFromRight(const Pointer node) const {
		Pointer tmp = node->link[1];
	...
	template<typename Key, typename Value, typename ValueEqual>
	void SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::ExcludeNode(Pointer a, Pointer b) {
	...
	template<typename Key, typename Value, typename ValueEqual>
	void SelfBalancingBinarySearchTree<Key, Value, ValueEqual>::BalanceTree(Pointer node) {
		Pointer uncle;
