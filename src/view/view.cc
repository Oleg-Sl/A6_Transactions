...
namespace s21 {
	...
	std::string View::GetCommand(Vector params) {
	...
	void View::CallSet(Vector params) {
		...
		if (params.size() == 7) {
			...
			res = controller_.Set(Data(params[1], Student(params[2], params[3], birth.second, params[5], coins.second)));
		}
		...
		if (params.size() == 9) {
			...
			res = controller_.Set(Data(params[1], Student(params[2], params[3], birth.second, params[5], coins.second), validity.second));
		}
	...
	std::pair<bool, int> View::ParseToInt(std::string str) {
		try {
			...
			std::cout << "ERROR: unable to cast value " << str << " to type int" << std::endl;
